    ...
    T: Float + std::ops::AddAssign + Display,
{
    ...
    pub fn robustness_uniform(&mut self, diff_ratio: f64) {
        let mut rng = thread_rng();
        for (_ev_number, gene) in &self.genes_ev_number {
            let mut gene_cp = gene.borrow_mut();
            let min: f64 = -diff_ratio * gene_cp.input_weight.to_f64().unwrap() * gene_cp.input_weight.to_f64().unwrap().signum();
            let max: f64 = diff_ratio * gene_cp.input_weight.to_f64().unwrap() * gene_cp.input_weight.to_f64().unwrap().signum();
            let uniform = Uniform::from(min..max);
            gene_cp.input_weight += T::from(uniform.sample(&mut rng)).unwrap();
            let min: f64 = -diff_ratio * gene_cp.memory_weight.to_f64().unwrap() * gene_cp.memory_weight.to_f64().unwrap().signum();
            let max: f64 = diff_ratio * gene_cp.memory_weight.to_f64().unwrap() * gene_cp.memory_weight.to_f64().unwrap().signum();
            let uniform = Uniform::from(min..max);
            gene_cp.memory_weight += T::from(uniform.sample(&mut rng)).unwrap();
            let min: f64 = -diff_ratio * gene_cp.reset_input_weight.to_f64().unwrap() * gene_cp.reset_input_weight.to_f64().unwrap().signum();
            let max: f64 = diff_ratio * gene_cp.reset_input_weight.to_f64().unwrap() * gene_cp.reset_input_weight.to_f64().unwrap().signum();
            let uniform = Uniform::from(min..max);
            gene_cp.reset_input_weight += T::from(uniform.sample(&mut rng)).unwrap();
            let min: f64 = -diff_ratio * gene_cp.update_input_weight.to_f64().unwrap() * gene_cp.update_input_weight.to_f64().unwrap().signum();
            let max: f64 = diff_ratio * gene_cp.update_input_weight.to_f64().unwrap() * gene_cp.update_input_weight.to_f64().unwrap().signum();
            let uniform = Uniform::from(min..max);
            gene_cp.update_input_weight += T::from(uniform.sample(&mut rng)).unwrap();
            let min: f64 = -diff_ratio * gene_cp.reset_memory_weight.to_f64().unwrap() * gene_cp.reset_memory_weight.to_f64().unwrap().signum();
            let max: f64 = diff_ratio * gene_cp.reset_memory_weight.to_f64().unwrap() * gene_cp.reset_memory_weight.to_f64().unwrap().signum();
            let uniform = Uniform::from(min..max);
            gene_cp.reset_memory_weight += T::from(uniform.sample(&mut rng)).unwrap();
            let min: f64 = -diff_ratio * gene_cp.update_memory_weight.to_f64().unwrap() * gene_cp.update_memory_weight.to_f64().unwrap().signum();
            let max: f64 = diff_ratio * gene_cp.update_memory_weight.to_f64().unwrap() * gene_cp.update_memory_weight.to_f64().unwrap().signum();
            let uniform = Uniform::from(min..max);
            gene_cp.update_memory_weight += T::from(uniform.sample(&mut rng)).unwrap();
        }
        for (_point, gene_and_bias) in &mut self.genes_point {
            let min: f64 = -diff_ratio * gene_and_bias.bias.bias_input.to_f64().unwrap() * gene_and_bias.bias.bias_input.to_f64().unwrap().signum();
            let max: f64 = diff_ratio * gene_and_bias.bias.bias_input.to_f64().unwrap() * gene_and_bias.bias.bias_input.to_f64().unwrap().signum();
            let uniform = Uniform::from(min..max);
            gene_and_bias.bias.bias_input += T::from(uniform.sample(&mut rng)).unwrap();
            let min: f64 = -diff_ratio * gene_and_bias.bias.bias_update.to_f64().unwrap() * gene_and_bias.bias.bias_update.to_f64().unwrap().signum();
            let max: f64 = diff_ratio * gene_and_bias.bias.bias_update.to_f64().unwrap() * gene_and_bias.bias.bias_update.to_f64().unwrap().signum();
            let uniform = Uniform::from(min..max);
            gene_and_bias.bias.bias_update += T::from(uniform.sample(&mut rng)).unwrap();
            let min: f64 = -diff_ratio * gene_and_bias.bias.bias_reset.to_f64().unwrap() * gene_and_bias.bias.bias_reset.to_f64().unwrap().signum();
            let max: f64 = diff_ratio * gene_and_bias.bias.bias_reset.to_f64().unwrap() * gene_and_bias.bias.bias_reset.to_f64().unwrap().signum();
            let uniform = Uniform::from(min..max);
            gene_and_bias.bias.bias_reset += T::from(uniform.sample(&mut rng)).unwrap();
        }
        for bias in &mut self.output_bias {
            let min: f64 = -diff_ratio * bias.bias_input.to_f64().unwrap() * bias.bias_input.to_f64().unwrap().signum();
            let max: f64 = diff_ratio * bias.bias_input.to_f64().unwrap() * bias.bias_input.to_f64().unwrap().signum();
            let uniform = Uniform::from(min..max);
            bias.bias_input += T::from(uniform.sample(&mut rng)).unwrap();
            let min: f64 = -diff_ratio * bias.bias_update.to_f64().unwrap() * bias.bias_update.to_f64().unwrap().signum();
            let max: f64 = diff_ratio * bias.bias_update.to_f64().unwrap() * bias.bias_update.to_f64().unwrap().signum();
            let uniform = Uniform::from(min..max);
            bias.bias_update += T::from(uniform.sample(&mut rng)).unwrap();
            let min: f64 = -diff_ratio * bias.bias_reset.to_f64().unwrap() * bias.bias_reset.to_f64().unwrap().signum();
            let max: f64 = diff_ratio * bias.bias_reset.to_f64().unwrap() * bias.bias_reset.to_f64().unwrap().signum();
            let uniform = Uniform::from(min..max);
            bias.bias_reset += T::from(uniform.sample(&mut rng)).unwrap();
        }
    }

    pub fn robustness_normal(&mut self, diff_ratio: f64) {
        let mut rng = thread_rng();
        for (_ev_number, gene) in &self.genes_ev_number {
            let mut gene_cp = gene.borrow_mut();
            let normal = Normal::new(gene_cp.input_weight.to_f64().unwrap(),  diff_ratio * gene_cp.input_weight.to_f64().unwrap().abs()).unwrap();
            gene_cp.input_weight += T::from(normal.sample(&mut rng)).unwrap();
            let normal = Normal::new(gene_cp.memory_weight.to_f64().unwrap(), diff_ratio * gene_cp.memory_weight.to_f64().unwrap().abs()).unwrap();
            gene_cp.memory_weight += T::from(normal.sample(&mut rng)).unwrap();
            let normal = Normal::new(gene_cp.reset_input_weight.to_f64().unwrap(), diff_ratio * gene_cp.reset_input_weight.to_f64().unwrap().abs()).unwrap();
            gene_cp.reset_input_weight += T::from(normal.sample(&mut rng)).unwrap();
            let normal = Normal::new(gene_cp.update_input_weight.to_f64().unwrap(), diff_ratio * gene_cp.update_input_weight.to_f64().unwrap().abs()).unwrap();
            gene_cp.update_input_weight += T::from(normal.sample(&mut rng)).unwrap();
            let normal = Normal::new(gene_cp.reset_memory_weight.to_f64().unwrap(), diff_ratio * gene_cp.reset_memory_weight.to_f64().unwrap().abs()).unwrap();
            gene_cp.reset_memory_weight += T::from(normal.sample(&mut rng)).unwrap();
            let normal = Normal::new(gene_cp.update_memory_weight.to_f64().unwrap(), diff_ratio * gene_cp.update_memory_weight.to_f64().unwrap().abs()).unwrap();
            gene_cp.update_memory_weight += T::from(normal.sample(&mut rng)).unwrap();
        }

        for (_point, gene_and_bias) in &mut self.genes_point {
            let normal = Normal::new(gene_and_bias.bias.bias_input.to_f64().unwrap(), diff_ratio * gene_and_bias.bias.bias_input.to_f64().unwrap().abs()).unwrap();
            gene_and_bias.bias.bias_input += T::from(normal.sample(&mut rng)).unwrap();
            let normal = Normal::new(gene_and_bias.bias.bias_update.to_f64().unwrap(), diff_ratio * gene_and_bias.bias.bias_update.to_f64().unwrap().abs()).unwrap();
            gene_and_bias.bias.bias_update += T::from(normal.sample(&mut rng)).unwrap();
            let normal = Normal::new(gene_and_bias.bias.bias_reset.to_f64().unwrap(), diff_ratio * gene_and_bias.bias.bias_reset.to_f64().unwrap().abs()).unwrap();
            gene_and_bias.bias.bias_reset += T::from(normal.sample(&mut rng)).unwrap();
        }
        for bias in &mut self.output_bias {
            let normal = Normal::new(bias.bias_input.to_f64().unwrap(), diff_ratio * bias.bias_input.to_f64().unwrap().abs()).unwrap();
            bias.bias_input += T::from(normal.sample(&mut rng)).unwrap();
            let normal = Normal::new(bias.bias_update.to_f64().unwrap(), diff_ratio * bias.bias_update.to_f64().unwrap().abs()).unwrap();
            bias.bias_update += T::from(normal.sample(&mut rng)).unwrap();
            let normal = Normal::new(bias.bias_reset.to_f64().unwrap(), diff_ratio * bias.bias_reset.to_f64().unwrap().abs()).unwrap();
